...
func main() {
	a := app.New()
	logoResource := fyne.NewStaticResource("cleanpulse-logo.png", appLogoPNG)
	a.SetIcon(logoResource)
	w := a.NewWindow(fmt.Sprintf("CleanPulse %s", buildinfo.Version))
	...
	scanView = container.NewBorder(
		container.NewVBox(
			widget.NewLabel(fmt.Sprintf("CleanPulse GUI %s", buildinfo.Version)),
	...
	homeView = container.NewBorder(
		container.NewVBox(widget.NewLabel(fmt.Sprintf("CleanPulse GUI %s", buildinfo.Version))),
		nil,
		nil,
		nil,
...
func guiHealthStatePath() (string, error) {
	...
	return filepath.Join(home, ".cleanpulse", "gui-health.json"), nil
}
...
func buildCleanupView(parent fyne.Window, onCleanupFinished func(devcleanup.RunReport)) (fyne.CanvasObject, fyne.CanvasObject) {
	...
	scheduleStateEntry.SetText(".cleanpulse/scheduler-state.json")
	scheduledReportDirEntry := widget.NewEntry()
	scheduledReportDirEntry.SetText(".cleanpulse/reports")
